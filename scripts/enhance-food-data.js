...
function enhanceFoodRecord(shop) {
  const defaultHours = DEFAULT_HOURS[shop.type] || { open: '10:00', close: '22:00' }
...
// T005: 批量数据增强和文件输出
console.log('📂 正在加载原始数据...')

const rawData = JSON.parse(
  fs.readFileSync('src/assets/data/guiyang_food_data.json', 'utf-8')
)

console.log(`✅ 原始数据加载成功：${rawData.length} 条记录`)

const enhancedData = rawData.map(enhanceFoodRecord)
...
console.log(`\n🎉 数据增强完成！`)
console.log(`📊 增强数据：${enhancedData.length} 条记录`)
